export default function ImportOverlay({ overlay }) {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-30 w-[300px] rounded-lg border border-border bg-panel/95 p-3.5 shadow-overlay backdrop-blur-md">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-[11px] font-medium text-muted">Import</div>
          <div className="mt-1 text-[13px] font-medium leading-snug text-text">{overlay.title}</div>
        </div>
        {overlay.percent ? (
          <div className="shrink-0 rounded bg-accent/15 px-2 py-0.5 text-[12px] font-medium tabular-nums text-accent">{overlay.percent}</div>
        ) : null}
      </div>
